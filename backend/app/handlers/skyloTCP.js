...
function parseHexPayload(raw) {
    ...
    for (let r = 0; r < recordCount; r++) {
        ...
        records.push({
            ...
            timestamp:  u32le(hex, off + 1) > 0
                            ? new Date(u32le(hex, off + 1) * 1000).toISOString()
...
async function handlePayload(raw, remoteAddress) {
    const cleaned = raw.toString('utf8').trim();
    console.log(`📡 [Skylo] Received from ${remoteAddress}: ${cleaned}`);

    let records;
    try {
        records = parseHexPayload(cleaned);
    } catch (err) {
        console.error(`❌ [Skylo] Parse error from ${remoteAddress}: ${err.message}`);
        return;
    }
    ...
    for (const record of records) {
        try {
            ...
            console.error(`❌ [Skylo] DB insert error for device_id=${record.device_id}: ${err.message}`);
        }
    }
}

const TCP_PORT = process.env.SKYLO_TCP_PORT || 5005;

const tcpServer = net.createServer((socket) => {
    const remoteAddress = `${socket.remoteAddress}:${socket.remotePort}`;
    console.log(`🔌 [Skylo] New TCP connection from ${remoteAddress}`);
    ...
    socket.on('data', (data) => {
        ...
        if (trimmed.length > 0 && trimmed.length % 36 === 0) {
            handlePayload(trimmed, remoteAddress);
            buffer = '';
        }
    });
    ...
    socket.on('end', () => {
        if (buffer.trim().length > 0) {
            handlePayload(buffer.trim(), remoteAddress);
            buffer = '';
        }
        console.log(`🔌 [Skylo] Connection closed: ${remoteAddress}`);
    });

    socket.on('error', (err) => {
        console.error(`❌ [Skylo] Socket error from ${remoteAddress}: ${err.message}`);
    });

    socket.setTimeout(30000);
    socket.on('timeout', () => {
        console.warn(`⏰ [Skylo] Connection timeout: ${remoteAddress}`);
        socket.destroy();
    });
});

tcpServer.on('error', (err) => {
    console.error(`❌ [Skylo] TCP server error: ${err.message}`);
});

tcpServer.listen(TCP_PORT, '0.0.0.0', () => {
    console.log(`🛰️  [Skylo] TCP listener running on port ${TCP_PORT}`);
});
